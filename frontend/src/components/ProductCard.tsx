...
export function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="product-card">
    <Link to={`/producto/${product.slug}`} className="product-image">
      ...
      <span className="product-orbit" aria-hidden="true">✦</span>
    </Link>
    <div className="product-info">
      ...
      <div className="product-bottom">
        ...
        <button disabled={product.stock === 0} onClick={() => cart.add(product)} aria-label={`Agregar ${product.name}`}><BagIcon /><span>Agregar</span></button>
      </div>
    </div>
  </article>
}
